...
if ($included && $inline && array_key_exists('pic'.$section_id, $_GET)) {
	...
	if ($showarrows == 'yes'){
		if ($pic > 0) {
			if ($textlink==1){
				echo '<a href="'.html($url).'pic'.$section_id.'='.($pic-1).html($urlsuffix).'">['.$MOD_IMAGEGALLERY['PREVIOUS'].']</a>';	
			} else {
				echo '<a href="'.html($url).'pic'.$section_id.'='.($pic-1).html($urlsuffix).'" title="'.$MOD_IMAGEGALLERY['PREVIOUS'].'"><img src="'.WB_URL.'/modules/imagegallery/images/left_16.png" border="0" alt="'.$MOD_IMAGEGALLERY['PREVIOUS'].'" /></a>';
			}
		}
	}
	if ($pic >= $maxpics) {
	...
	if ($showarrows == 'yes'){
		if ($textlink == 1) {
			echo '<a href="'.html($u).'">['.$MOD_IMAGEGALLERY['OVERVIEW'].']</a>';
		} else {
			echo '<a href="'.html($u).'" title="'.$MOD_IMAGEGALLERY['OVERVIEW'].'"><img src="'.WB_URL.'/modules/imagegallery/images/up_16.png" border="0" alt="'.$MOD_IMAGEGALLERY['OVERVIEW'].'" /></a>';
		}
		if ($pic + 1 < sizeof($pics)){
			if ($textlink == 1){
					echo '<a href="'.html($url).'pic'.$section_id.'='.($pic + 1).html($urlsuffix).'">['.$MOD_IMAGEGALLERY['NEXT'].']</a>';
			} else {
					echo '<a href="'.html($url).'pic'.$section_id.'='.($pic + 1).html($urlsuffix).'" title="'.$MOD_IMAGEGALLERY['NEXT'].'"><img src="'.WB_URL.'/modules/imagegallery/images/right_16.png" border="0" alt="'.$MOD_IMAGEGALLERY['NEXT'].'" /></a>';
			}
		}
	}
	echo "\n".'</div>'."\n";
